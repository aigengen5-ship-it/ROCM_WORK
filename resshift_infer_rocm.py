...
# ======================================================================================
# [ROCm / Flash Attention / SageAttention 최적화 패치]
# 벤더(Vendor) 모듈이 로드되기 전에 xformers를 가로채어 AMD 호환 Attention으로 우회합니다.
# ======================================================================================
try:
...
def _optimized_memory_efficient_attention(query, key, value, attn_bias=None, p=0.0, scale=None, op=None):
    # xformers 입력 형태: (Batch, SeqLen, Heads, HeadDim)
    # PyTorch SDPA / SageAttention 형태: (Batch, Heads, SeqLen, HeadDim)
    q = query.transpose(1, 2)
    k = key.transpose(1, 2)
    v = value.transpose(1, 2)
    
    # 1. Sage Attention 시도 (설치되어 있고, dropout/bias가 없으며 fp16/bf16일 때만 동작)
    if HAS_SAGE and attn_bias is None and p == 0.0 and q.dtype in (torch.float16, torch.bfloat16):
        try:
            out = sageattn(q, k, v, is_causal=False, scale=scale)
            return out.transpose(1, 2).contiguous()
        except Exception:
            pass  # 차원수(HeadDim) 등의 이유로 SageAttention이 실패하면 아래의 Flash Attention으로 폴백
    
    # 2. PyTorch Native SDPA (ROCm 환경에서 자동으로 Flash Attention 커널로 스위칭 됨)
...
try:
    import xformers.ops
    xformers.ops.memory_efficient_attention = _optimized_memory_efficient_attention
except ImportError:
    xformers_mock = types.ModuleType("xformers")
    xformers_ops_mock = types.ModuleType("xformers.ops")
    xformers_ops_mock.memory_efficient_attention = _optimized_memory_efficient_attention
...
def upscale(cfg, diff, vqgan, student, lr_m1p1, device, scale, align,
    ...
    flat regions. Instead we draw ONE 실 full-image latent noise field per source and slice
